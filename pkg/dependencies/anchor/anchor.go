...
func (r *runtime) BuildProgram(code string) ([]byte, error) {
	...
	newCmd.Dir = "./pkg/dependencies/runtimes/anchor/code-exec"
	if output, err := newCmd.CombinedOutput(); err != nil {
		log.Println(err)
		return nil, fmt.Errorf("error running Anchor new: %s", string(output))
	}
	defer os.RemoveAll("./pkg/dependencies/runtimes/anchor/code-exec/programs/" + id)

	err := os.WriteFile(fmt.Sprintf("./pkg/dependencies/runtimes/anchor/code-exec/programs/%s/src/lib.rs", id), []byte(code), 0644)
	if err != nil {
		return nil, err
	}

	defer os.RemoveAll("./pkg/dependencies/runtimes/anchor/code-exec/target/deploy")
	cmd := exec.Command("anchor", "build", "--program-name", id)
	cmd.Dir = "./pkg/dependencies/runtimes/anchor/code-exec"
	if output, err := cmd.CombinedOutput(); err != nil {
		log.Println(err)
		return nil, fmt.Errorf("error building Anchor: %s", string(output))
	}
	outputFile := fmt.Sprintf("./pkg/dependencies/runtimes/anchor/code-exec/target/deploy/%s.so", id)
